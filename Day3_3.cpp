...
class treenode
{
	...
	int imbalance(treenode *node)
	{
		...
		{
			if((((node->l)->c)-((node->r)->c)==2))
				return 1;
			else if((((node->l)->c)-((node->r)->c)==-2))
				return -1;
			else return 0;
		}
...
void treenode::rebalance(treenode *node)
{
	treenode *newadd=NULL, *t=NULL, *temp=node;
	while(temp!=NULL)
	{	
		if(imbalance(node)==-1)
		{
			node->l=tryinsert(node->l,node->v,&newadd);
			incrcount(newadd);
			newadd=NULL;
			t=successor(node);
			node->v=t->v;
			node->r=trydelete(node->r,node->v,&newadd);
			decrcount(newadd);
			newadd=NULL;
			temp=temp->p;
		}
		else if(imbalance(node)==1)
		{	
			node->r=tryinsert(node->r,node->v,&newadd);
			incrcount(newadd);
			newadd=NULL;
			t=predecessor(node);
			node->v=t->v;
			node->l=trydelete(node->l,node->v,&newadd);
			decrcount(newadd);
			newadd=NULL;
			temp=temp->p;
		}
...
treenode * treenode::predecessor(treenode *x)
{
	if(x->l!=NULL)
		while((x->l)->r!=NULL)x->l=(x->l)->r;
	treenode *y=x->p;
	while(y!=NULL && x==(y->l))
	{
		x=y;
